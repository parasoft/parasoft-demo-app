...
function initHeaderController(app){
	...
	app.controller('headerController', function($rootScope, $http, $filter) {
		...
		resetHeaderByBrowserWindowSize($rootScope);
		listenBrowserResizeforHeader();
	});
}
...
function initRequisitionBarController(app){
	app.controller('requisitionBarController', function($rootScope, $http, $filter, $location) {
		...
		req.closeRequisition = function(){
			...
			angular.element("#right_navigation_bar").removeClass("in");
		}
		...
		$rootScope.switchBar = function(){
			...
			var status = angular.element("#right_navigation_bar").attr("class");
	...
	setTimeout(function(){
		$http({
	    	...
	    	for(var i = 0; i < data.length; i++){
	    		...
	    		//load cart items' format
	    		angular.element(".requisition_title_row"+number).css("top",125 * i + "px");
	    		angular.element(".requisition_img_row"+number).css("top",27 + 125 * i + "px");
	    		angular.element(".requisition_description_row"+number).css("top",27 + 125 * i + "px");
	    		angular.element(".requisition_number_row"+number).css("top",27 + 125 * i + "px");
	    		angular.element(".requisition_error"+number).css("top",33 + 125 * i + "px");
	    		angular.element(".requisition_minus_row"+number).css("top",62 + 125 * i + "px");
	    		angular.element(".requisition_plus_row"+number).css("top",62 + 125 * i + "px");
	    		angular.element(".requisition_remove_row"+number).css("top",67 + 125 * i + "px");
...
var scrollPosition, scrollDistance;
function listenBrowserResizeforHeader(){
	//Trigger by using scroll bar
	window.onscroll=function(){
		scrollDistance = Math.max(document.body.scrollLeft,document.documentElement.scrollLeft);
		scrollDistance = Math.floor(scrollDistance);
		if(scrollDistance !== 0){
			setTimeout(function(){
				scrollPosition = 600 - scrollDistance + 'px';
				document.getElementById('top_side').style.left = scrollPosition;
			}, 10);
		}else if(document.documentElement.clientWidth < 1269){
			$('top_side').attr("style","left:600px");
		}
	}
	
	//Trigger by resize window of browser, including maximization and minimality
	$(window).resize(function (){
		...
		if(document.documentElement.clientWidth > 1150){
			document.getElementById('top_side').style.left = 50+'%';
		}else if(scrollDistance === 0 && document.documentElement.clientWidth < 1150){
			document.getElementById('top_side').style.left = 600+'px';
		}else {
			document.getElementById('top_side').style.left = scrollPosition;
		}
	});
...
function resetHeaderByBrowserWindowSize($rootScope){
	scrollDistance = Math.max(document.body.scrollLeft,document.documentElement.scrollLeft);
	scrollDistance = Math.floor(scrollDistance);
	if(scrollDistance !== 0){
		scrollPosition = 600 - scrollDistance +'px'
